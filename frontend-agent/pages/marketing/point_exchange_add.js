...
Page({
  ...
  submit: function (e) {
    ...
    var single_limit = this.data.counts[e.detail.value.single_limit]
    if ('0' == coupon_id) {
      wx.showModal({
        title: "请选择优惠券",
        content: "",
    ...
    wx.request({
      url: host + 'ssh_marketing.php?action=point_exchange_add',
      data: {
        mch_id: wx.getStorageSync('mch_id'),
        point: point,
        coupon_id: coupon_id,
  ...
  bindCountChange: function (e) {
    ...
    this.setData({
      ...
      single_limit: that.data.counts[e.detail.value]+'张'
    })
  },
