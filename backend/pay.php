<?php
  ...
  switch ($action) {
    ...
    case 'selfpay':
      ...
      $member = $ret ? $ret : array('cardnum'=>'');

      $pointRules = unserialize($redis->hget('keyou_mch_point_rules', $subMchId));
      $getPoint = floor($trade / $pointRules['award_need_consume']);

      echo json_encode(array('key'=>$scene, 'get_point'=>$getPoint, 'member'=>$member, 'can_cash'=>$pointRules['can_cash']));
      break;
    case 'get_detail':
      $key = $_GET['key'];
      ...
      $isMember  = true;
      $grade     = $_GET['grade'];
      $payAction = $_GET['pay_action']; //scan扫收银员码买单,self顾客自助买单,waimai外卖
    ...
    case 'refresh_detail':
      ...
      $payAction = $_GET['pay_action']; //scan扫收银员码买单,self顾客自助买单
      $pointSpeed = 1;
    ...
    case 'getPrepay':
      ...
      $payAction= isset($_GET['pay_action']) ? $_GET['pay_action']:'';
      ...
      if ('suishouhui' == $payPlatForm) {
        $jsApiArgs = postUnifiedorder($subOpenId, $mchId, $body, $attach, $totalFee);
      } else if ('tenpay' == $payPlatForm) {
        $sql = "SELECT * FROM mch_tenpay_configs WHERE mch_id = $mchId";
        $tenpayConfig = $db->fetch_row($sql);
        $jsApiArgs = postTenpayUnifiedorder($tenpayConfig, $subOpenId, $mchId, $body, $attach, $totalFee);
      }
    ...
    case 'consume_no_money':
      ...
      if ('scan' == $payAction) {
        ...
        $url     = SAE_URL.'/send_message.php?channel='.$key.'&message='.$pushMessage;
        $ret     = httpPost($url);
      }
    ...
    case 'send_websocket_message':
      ...
      $url     = SAE_URL.'/send_message.php?channel='.$channel.'&message='.$message;
      $ret     = httpPost($url);
      echo $ret;
      break;
    default:
