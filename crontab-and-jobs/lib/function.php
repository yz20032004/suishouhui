<?php
  ...
  function httpSSLPost($postUrl, $data='')
  {
    $ch = curl_init();
    // 设置URL和相应的选项
    curl_setopt($ch, CURLOPT_URL, $postUrl);
  ...
  {
    $outTradeNo = date('ymdHi').rand(10000,99999);
    return $outTradeNo;
  }
  ...
  //客友信息直连商户号操作
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, __DIR__.'/keyou_cert/apiclient_cert.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, __DIR__.'/keyou_cert/apiclient_key.pem');
		}
		//post提交方式
	...
	function postXmlCurlByFormData($xml, $url, $useCert = false, $second = 30)
	{		
        //初始化curl        
       	$ch = curl_init();
		//设置超时
