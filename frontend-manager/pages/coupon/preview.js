...
Page({
  ...
  onLoad: function (options) {
    ...
    var id = options.id
    wx.request({
      url: host + 'coupon.php?action=get_detail',
      data: {
    ...
    wx.request({
      url: host + 'coupon.php?action=get_icon_url',
      data: {
        id: id,
      },
      header: {
  ...
  submit:function(e){
    ...
    wx.request({
      url: host + 'coupon.php?action=update_balance',
      data: {
        mch_id: wx.getStorageSync('mch_id'),
  ...
  edit:function(){
    var that = this
    wx.showActionSheet({
      itemList: ['投放优惠券', '增加库存', '添加图片', '作废'],
      success(res) {
        if (0 == res.tapIndex) {
          if ('groupon' == that.data.coupon.coupon_type) {
            wx.navigateTo({
              url: '../buy/add',
            })
          } else {
  ...
  disableCoupon:function(){
    var coupon_id = this.data.coupon.id
    wx.request({
      url: host + 'coupon.php?action=disable',
      data: {
        mch_id: wx.getStorageSync('mch_id'),
        id: coupon_id,
      },
