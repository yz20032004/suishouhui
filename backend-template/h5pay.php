<?php
  ...
  $sql = "SELECT mch_id, merchant_name, counter, name FROM app_counters WHERE counter = '$counter'";
...
<script type="text/javascript">
    ...
    ap.onSocketMessage(function(res) {
      ap.showToast('get message '+res.data)
      if ('complete' == res.data.pay) {
        //ap.closeSocket();
        ap.popWindow();
      }
    })*/
      ...
      $.get(url, function (result, status) {
            if ('success' == status) {
              ...
              /*var channel = 'alipay_'+mch_id+out_trade_no
              $.get('ssh_websocket.php?action=create_channel&channel='+channel, function(socket_result, socket_status){
                ap.connectSocket({
                  url: socket_result.data,
                });
              }, 'json')*/
              ap.pushWindow({
                url:pay_url
